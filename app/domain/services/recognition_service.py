...
class RecognitionService:
    ...
    @staticmethod
    def find_match(unknown_encoding, known_encodings_records, tolerance=0.5):
        if not known_encodings_records:
            return None, 0.0
            
        known_encodings = [np.frombuffer(record.encoding, dtype=np.float64) for record in known_encodings_records]
        ...
        if len(face_distances) > 0:
            ...
            if matches[best_match_index]:
                ...
                return known_encodings_records[best_match_index].student_id, confidence
